...
export function Navbar({ content, roleKey }: NavbarProps) {
  ...
  const logoRef = useRef<HTMLAnchorElement>(null) as React.RefObject<HTMLAnchorElement>
  const resumeRef = useRef<HTMLButtonElement>(null) as React.RefObject<HTMLButtonElement>
  const themeToggleRef = useRef<HTMLDivElement>(null) as React.RefObject<HTMLDivElement>
  ...
  const getMagneticStyle = (elementRef: React.RefObject<HTMLElement>) => {
    ...
    if (distance < 100) {
      ...
      return {
        transform: `translate(${deltaX * strength * 0.1}px, ${deltaY * strength * 0.1}px)`,
      }
    }
    return {}
  }
        ...
        <div className="container mx-auto max-w-7xl px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
                ...
                {sections.map((section) => (
                  ...
                  >
                    <span className="relative z-10 flex items-center space-x-2">
                      ...
                      <span className="transition-all duration-300 group-hover:translate-x-0.5">{section.name}</span>
                    </span>
                    {activeSection === section.id && (
            ...
            <div className="flex items-center space-x-3">
              <div ref={themeToggleRef} style={getMagneticStyle(themeToggleRef)}>
                <ModeToggle />
              </div>
              <Button
                ref={resumeRef}
          ...
          {sections.map((section, index) => (
            ...
            >
              <span className="flex items-center space-x-4 relative z-10">
                ...
                <span className="group-hover:translate-x-1 transition-transform duration-300">{section.name}</span>
              </span>
